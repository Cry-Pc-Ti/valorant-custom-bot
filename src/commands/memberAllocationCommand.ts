...
export const memberAllocationCommand = {
  ...
  execute: async (interaction: ChatInputCommandInteraction) => {
    ...
    try {
      ...
      let memberAllocation:MemberAllocationData = {
        attack: [],
        defense: [],
      }
      ...
      for(let i = 1; i < 11; i++){
        if(options.getUser('メンバー' + String(i)) !== null){
          ...
          if((randomIndex === 1 && memberAllocation.attack.length < 5) || (randomIndex === 2 && memberAllocation.defense.length >= 5)){
            memberAllocation.attack.push(member);
          }else if((randomIndex === 2 && memberAllocation.defense.length < 5) || (randomIndex === 1 && memberAllocation.attack.length >= 5)){
